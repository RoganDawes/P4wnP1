...
$LinkLayerClass = New-Object psobject -Property @{
...
#    ps_hid_out = [powershell]::Create() # powershell subrocess running HIDout thread
    ...
    globalstate = $null # Thread global data (synchronized hashtable for state)
    iss = $null # initial seesion state for threads (runspaces)
        
    ps_hid_in = $null # powershell subrocess running HIDin thread
    ps_hid_out = $null # powershell subrocess running HIDout thread
        
}

 # Create construcort / init method
$LinkLayerClass | Add-Member -MemberType ScriptMethod -Name "Init" -Value {
    param(
...
#    $this.globalstate.last_seq_received = 12 # last seq number read !! has to be exchanged between threads, thus defined in global RunSpace threadsafe hashtable
    ...
    $this.globalstate.streamReceivedEvent = New-Object System.Threading.AutoResetEvent $false
...
$LinkLayerClass | Add-Member -MemberType ScriptMethod -Name "Start" -Value {
#    param(
#
#        [Parameter(Mandatory=$true, Position=0)]
#        [Byte]
...
[Console]::WriteLine("Start SEQ {0}" -f $this.globalstate.last_valid_seq_received)
    ...
    $handle_hid_in = $this.ps_hid_in.BeginInvoke()

    # start HID out thread, idle loop till finish    
    $handle_hid_out = $this.ps_hid_out.BeginInvoke()
}


$LinkLayerClass | Add-Member -MemberType ScriptMethod -Name "Stop" -Value {
    # stop threads
    ...
    $this.ps_hid_in.Runspace.Dispose()
    $this.ps_hid_in.Dispose()
    
    $this.ps_hid_out.Stop()
    $this.ps_hid_out.Runspace.Dispose()
    $this.ps_hid_out.Dispose()
}

# method to get current input report queue size
$LinkLayerClass | Add-Member -MemberType ScriptMethod -Name "PendingInputStreamCount" -Value {
    $this.globalstate.report_in_queue.Count
}

# method to get current input report queue size
$LinkLayerClass | Add-Member -MemberType ScriptMethod -Name "WaitForInputStream" -Value {
    # we call waitOne with a time out in an endless loop
    # otherwise the thread couldn't be terminated while in wait state
    # the method returns ONLY IF A SIGNAL IS RECEIVED

    while ($true)
    {
        $res = $this.globalstate.streamReceivedEvent.WaitOne(100) # 100 ms timeout
        if ($res) { return }
    }
...
# method to get current input report queue size
$LinkLayerClass | Add-Member -MemberType ScriptMethod -Name "GetPayloadBytesReceived" -Value {
    $this.globalstate.payload_bytes_received
}

# method to get current input report queue size
$LinkLayerClass | Add-Member -MemberType ScriptMethod -Name "ResetPayloadBytesReceived" -Value {
    $this.globalstate.payload_bytes_received = 0
}

# method to get last input report from queue
$LinkLayerClass | Add-Member -MemberType ScriptMethod -Name "PopPendingInputStream" -Value {
    $this.globalstate.report_in_queue.Dequeue()
}


# method to enqueue output stream
$LinkLayerClass | Add-Member -MemberType ScriptMethod -Name "PushOutputStream" -Value {
    param(
...
#
$LinkLayerClass | Add-Member -MemberType ScriptMethod -Name "Connect" -Value {
...
function LinkLayer {
    ...
    $ll = $LinkLayerClass.psobject.Copy()

    $ll.globalstate =  [hashtable]::Synchronized(@{}) # Thread global data (synchronized hashtable for state)
    $ll.iss = [Management.Automation.Runspaces.InitialSessionState]::CreateDefault() # initial seesion state for threads (runspaces)
    $ll.ps_hid_in = [powershell]::Create() # powershell subrocess running HIDin thread
    $ll.ps_hid_out = [powershell]::Create() # powershell subrocess running HIDout thread
    
    $ll.init($HIDin, $HIDout)
